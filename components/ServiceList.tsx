...
const ServiceList = () => {
      ...
      timeout = setTimeout(() => {
        if (searchMode === 'keyword') {
          ...
          if (finalFiltered.length > 0) {
            // Create bounds that include all search results
            const bounds = new google.maps.LatLngBounds();
            ...
            if (hasValidCoordinates) {
              // Check if any of the search results are visible in the current viewport
              ...
              // Only adjust the map if needed (when no search results are visible)
  ...
  const handleMapMarkerClick = useCallback((store: Store, index: number) => {
    ...
    setTimeout(() => {
      ...
      if (noStoresFound && searchMode === 'autocomplete') {
        ...
        // Look in visible stores or filtered stores depending on what's displayed
        storeIndex = visibleStores.findIndex(
          s => s.lat === store.lat && s.lng === store.lng
        );
        
        // If not found in visible stores (might be outside viewport), check filtered stores
        if (storeIndex === -1) {
  ...
  const renderList = () => {
    ...
    // If not filtering by viewport, show all filtered stores
    ...
    return (
      <Box sx={{
        ...
        {/* Display a count of visible vs total when filtering by viewport */}
